...
describe("palindrome via push and pop", function () {
    ...
    it("aba true", function () {
        assert.strictEqual(arrays.checkPaliendrom(["a", "b", "a"]), true);
    });
    it("abc false", function () {
        assert.strictEqual(arrays.checkPaliendrom(["a", "b", "c"]), false);
    });
    it("rotator true", function () {
        assert.strictEqual(arrays.checkPaliendrom(["r", "o", "t", "a", "t", "o", "r"]), true);
    });
});
...
describe("reverse2String", function () {
    ...
    it("Fox_Brown_Quick", function () {
        assert.strictEqual(arrays.tranferToString(["Quick", "Brown", "Fox"] ), "Fox_Brown_Quick");
    });

    it("4_3_2_1", function () {
        assert.strictEqual(arrays.tranferToString([1, 2, 3, 4] ), "4_3_2_1");
    });
});
...
// Do this without using reverse method, instead use splice and slice methods.  */
 describe("ssReverse", function () {
     ...
     it("[1, 2, 3, 4]", function () {
         assert.deepStrictEqual(arrays.reverse([1, 2, 3, 4 ]), [4,3,2,1] );
     });
 });
